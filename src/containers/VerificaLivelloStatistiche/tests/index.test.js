...
import { mockStatisticheElaborateDocente } from 'common/testing-fixtures/verifiche-report';
...
import VerificaLivelloStatistiche, { VerificaLivelloStatisticheView } from '../index';
...
describe('<VerificaLivelloStatisticheView />', () => {
  it('componentDidMount se !isDocente', () => {
    const spy = jest.spyOn(VerificaLivelloStatisticheView.prototype, 'componentDidMount');
    const props = {
      ...mockProps,
      userAppData: {
    ...
    shallow(<VerificaLivelloStatisticheView {...props} />);
  ...
  it('stato iniziale', () => {
    const renderedComponent = shallow(
      <VerificaLivelloStatisticheView {...mockProps} />
    );
  ...
  it('studente selezionato', () => {
    ...
    const renderedComponent = shallow(
      <VerificaLivelloStatisticheView {...props} />
    );
    expect(renderedComponent.find(TopBar).length).toBe(1);
    expect(renderedComponent.find(Spinner).length).toBe(0);
    expect(renderedComponent.find(AlertBanner).length).toBe(0);
  ...
  it('Controllo il funzionamento del back se docente === true', () => {
    ...
    expect(renderedComponent.find(TopBar).props().backNav.url).toBe('/verifiche-livello-docente/1000');
    expect(renderedComponent).toMatchSnapshot();
  });
  ...
  it('Controllo il funzionamento del back se docente === false', () => {
    ...
    expect(renderedComponent.find(TopBar).props().backNav.url).toBe('/verifiche-livello/1000');
    expect(renderedComponent).toMatchSnapshot();
  });
      ...
      contenuto: (
        <ZendeskTicket
          ticketData={{
            ...
            adminUrl: `${API_BASE_PATH}admin/cicero_academy/assegnazioneverifica/123/change`,
            adminURI: '/verifiche-livello-statistiche/1000/123',
  ...
  it('prev e next devono chiamare loadStat', () => {
    ...
    expect(spy).toHaveBeenCalledTimes(1);
    expect(spy).toHaveBeenCalledWith(412);

    // next
    renderedComponent.find(Container).find(FlexBox).at(0).find(GhostButton).at(1).simulate('click');
    expect(spy).toHaveBeenCalledTimes(2);
    expect(spy).lastCalledWith(214);
  });
});

describe('<VerificaLivelloStatistiche />', () => {
  ...
  it('testo il corretto funzionemento delle funzioni del mapDispatchToProps', () => {
    ...
    expect(mockStore.dispatch).toHaveBeenCalledWith(
      verificaLivelloStatisticheDataFetch(1, 2, true)
    );

    receivedProps.onSetDataStatistiche({ data: 123 });
    expect(mockStore.dispatch).toHaveBeenCalledWith(
      verificaLivelloStatisticheDataSet({ data: 123 })
    );
    ...
    expect(mockStore.dispatch).toHaveBeenCalledWith(
      verificaLivelloStatisticheDataSelect({ data: 123 })
    );

    receivedProps.onDidascaliaSet({ data: 123 });
    expect(mockStore.dispatch).toHaveBeenCalledWith(
      verificaLivelloStatisticheDidascaliaSet({ data: 123 })
    );
